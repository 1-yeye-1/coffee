...
export function useAnimeMotion() {
  ...
  const successCheck = (target) => run('success', target, { opacity: [0.45, 1], scale: [0.55, 1.18, 1], rotate: [-18, 0], duration: 620, ease: 'out(4)' })
  ...
  const floatEmpty = (target) => run('empty', target, { translateY: [0, -7], duration: 1800, alternate: true, loop: true, ease: 'inOutSine' })
  ...
  const flashRow = (target) => run('row', target, { opacity: [1, 0.58, 1], translateX: [0, 3, 0], duration: 620, ease: 'out(3)' })
