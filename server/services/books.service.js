...
function resolveBookCounts(payload = {}, fallback = {}) {
  const stock = toNonNegativeInt(payload.stock, fallback.stock || 0)
  const rawReservable = payload.reservableStock ?? payload.reservable_stock
  const reservableStock = rawReservable === undefined
    ? toNonNegativeInt(fallback.reservableStock, stock)
    : toNonNegativeInt(rawReservable, stock)
  if (reservableStock > stock) {
    throw Object.assign(new Error('可预约数不能大于库存'), { statusCode: 400 })
  }
  return { stock, reservableStock }
}

function isActiveBookReservationStatus(status) {
  return ['pending', 'confirmed'].includes(String(status || '').trim())
}

function normalizeBookReservationStatus(status) {
  const value = String(status || '').trim()
  return ['pending', 'confirmed', 'completed', 'cancelled'].includes(value) ? value : ''
}
...
export async function updateBook(id, payload) {
  ...
  await pool.execute(
    ...
    [next.slug, next.title, next.author, next.category || '', Number(next.rating) || 0,
      stock, reservableStock, next.status || 'available', next.coverTone || null, next.coverUrl || null,
...
export async function deleteOwnBookReview(reviewId, userId) {
  const connection = await pool.getConnection()
  try {
    await connection.beginTransaction()
    const [result] = await connection.execute(
      "UPDATE book_reviews SET status = 'hidden' WHERE id = ? AND user_id = ? AND status = 'published'",
      [reviewId, userId],
    )
    if (!result.affectedRows) { await connection.rollback(); return false }
    const [[review]] = await connection.execute(
      'SELECT book_id AS bookId, parent_id AS parentId FROM book_reviews WHERE id = ?', [reviewId],
    )
    if (!review) { await connection.rollback(); return false }
    const bookId = Number(review.bookId)
    if (Number(review.parentId || 0) > 0) {
      await connection.execute(
        "UPDATE book_reviews SET status = 'hidden' WHERE id = ? AND parent_id = ? AND status = 'published'",
        [reviewId, bookId],
      )
    }
    await writeAudit(userId, 'book.review.delete', 'books', { bookId, reviewId, parentId: review.parentId || null }, connection)
...
export async function createBookReservation(bookId, userId) {
  ...
  try {
    ...
    const [[book]] = await connection.execute(
      `SELECT b.id, b.title, b.stock, b.reservable_stock AS reservableStock, b.status, b.seat_id AS seatId, b.location_label AS locationLabel,
    ...
    if (!['available', 'active'].includes(book.status) || Number(book.reservableStock) < 1) throw Object.assign(new Error('当前图书暂不可预约'), { statusCode: 409 })
    ...
    await connection.execute('UPDATE books SET reservable_stock = GREATEST(reservable_stock - 1, 0) WHERE id = ?', [bookId])
    await connection.commit()
...
async function findAdminBookReservationById(id, bookId = null, connection = pool) {
  const params = [id]
  const bookClause = bookId ? 'AND br.book_id = ?' : ''
  if (bookId) params.push(bookId)
  const [rows] = await connection.execute(
    `SELECT br.id, br.reservation_no AS reservationNo, br.user_id AS userId, br.book_id AS bookId,
      b.slug, b.title, COALESCE(u.nickname, u.username) AS nickname, u.phone, u.email,
      br.seat_id AS seatId, s.code AS seatCode, s.name AS seatName,
      br.location_label AS locationLabel, br.status, br.reserved_at AS reservedAt,
      br.cancelled_at AS cancelledAt, br.updated_at AS updatedAt
     FROM book_reservations br
     INNER JOIN books b ON b.id = br.book_id
     INNER JOIN users u ON u.id = br.user_id
     LEFT JOIN seats s ON s.id = br.seat_id
     WHERE br.id = ? ${bookClause}
     LIMIT 1`,
    params,
  )
  return rows[0] || null
...
export async function listAdminBookReservations(bookId, query = {}) {
  const { page, pageSize, offset } = parsePagination(query, 10)
  const safeBookId = Number(bookId)
  if (!safeBookId) throw Object.assign(new Error('图书不存在'), { statusCode: 404 })
  const clauses = ['br.book_id = ?']
  const params = [safeBookId]
  const keyword = String(query.keyword || '').trim()
  const status = normalizeBookReservationStatus(query.status || 'all')
  if (keyword) {
    clauses.push('(br.reservation_no LIKE ? OR b.title LIKE ? OR u.nickname LIKE ? OR u.username LIKE ? OR u.phone LIKE ? OR u.email LIKE ?)')
    const like = `%${keyword}%`
  ...
  const [rows] = await pool.execute(
    `SELECT br.id, br.reservation_no AS reservationNo, br.user_id AS userId, br.book_id AS bookId,
     ...
     LEFT JOIN seats s ON s.id = br.seat_id
     ${where}
     ORDER BY br.reserved_at DESC, br.id DESC
     LIMIT ${Number(pageSize)} OFFSET ${Number(offset)}`,
    params,
  )
  return { items: rows, meta: { page, pageSize, total: Number(total) } }
}
...
export async function cancelBookReservation(id, userId) {
  ...
  try {
    ...
    await connection.execute('UPDATE books SET reservable_stock = LEAST(reservable_stock + 1, stock) WHERE id = ?', [reservation.bookId])
    await connection.commit()
    return true
  } catch (error) { await connection.rollback(); throw error } finally { connection.release() }
}

export async function updateAdminBookReservationStatus(bookId, reservationId, payload = {}, operatorId = null) {
  const nextStatus = normalizeBookReservationStatus(payload.status)
  const note = String(payload.reason || payload.note || '').trim()
  if (!nextStatus) throw Object.assign(new Error('图书预约状态无效'), { statusCode: 400 })
  const connection = await pool.getConnection()
  try {
    await connection.beginTransaction()
    const [[book]] = await connection.execute(
      'SELECT id, title, stock, reservable_stock AS reservableStock FROM books WHERE id = ? FOR UPDATE',
      [bookId],
    )
    if (!book) {
    ...
    const reservation = await findAdminBookReservationById(reservationId, bookId, connection)
    if (!reservation) throw Object.assign(new Error('预约记录不存在或不属于该图书'), { statusCode: 404 })
    if (reservation.status === nextStatus) {
      await connection.commit()
      return reservation
    }
    const wasActive = isActiveBookReservationStatus(reservation.status)
    const willBeActive = isActiveBookReservationStatus(nextStatus)
    let nextReservableStock = Number(book.reservableStock || 0)
    if (!wasActive && willBeActive) {
      if (nextReservableStock < 1) throw Object.assign(new Error('当前没有可用预约名额，无法恢复为进行中状态'), { statusCode: 409 })
      nextReservableStock -= 1
    }
    if (wasActive && !willBeActive) {
      nextReservableStock = Math.min(nextReservableStock + 1, Number(book.stock || 0))
    }
    await connection.execute(
      `UPDATE book_reservations
       SET status = ?, cancelled_at = ${nextStatus === 'cancelled' ? 'CURRENT_TIMESTAMP' : 'NULL'}, updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND book_id = ?`,
      [nextStatus, reservationId, bookId],
    )
    if (nextReservableStock !== Number(book.reservableStock || 0)) {
      await connection.execute('UPDATE books SET reservable_stock = ? WHERE id = ?', [nextReservableStock, bookId])
    }
    await writeAudit(operatorId, 'book.reservation.status.update', 'books', {
      id: bookId,
      bookId,
      reservationId,
    ...
    const title = nextStatus === 'cancelled'
      ? '图书预约已取消'
      : nextStatus === 'completed'
        ...
        : nextStatus === 'confirmed'
          ? '图书预约已确认'
          : '图书预约状态已更新'
    const content = nextStatus === 'cancelled'
      ? `你预约的图书《${book.title}》已由管理员取消。${note ? `备注：${note}` : ''}`
      : nextStatus === 'completed'
        ? `你预约的图书《${book.title}》已完成处理。${note ? `备注：${note}` : ''}`
        : `你预约的图书《${book.title}》状态已更新为 ${nextStatus === 'confirmed' ? '已确认' : '待处理'}。${note ? `备注：${note}` : ''}`
    ...
    await connection.commit()
    return findAdminBookReservationById(reservationId, bookId)
  } catch (error) {
    await connection.rollback()
    throw error
  } finally {
