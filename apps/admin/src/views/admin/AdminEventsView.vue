...
const columns = [
  { key: 'visual', label: '海报' },
  { key: 'primary', label: '活动标题' },
  { key: 'date', label: '日期' },
  { key: 'time', label: '时间' },
  { key: 'attendees', label: '报名人数' },
...
const fields = [
  ...
  { key: 'attendees', label: '报名人数', type: 'number' },
...
const stats = [
  ...
  { label: '开放报名', icon: 'Y', value: (items) => items.filter((item) => ['published', 'ongoing', 'open'].includes(item.status)).length, hint: '当前可报名' },
  { label: '报名人数', icon: '+', value: (items) => items.reduce((sum, item) => sum + Number(item.attendees || 0), 0), hint: '累计报名' },
  { label: '报名提醒', icon: '!', value: (items) => items.filter((item) => Number(item.capacity || 0) > 0 && Number(item.attendees || 0) >= Number(item.capacity || 0) * 0.8).length, hint: '报名达到容量 80%' },
]
</script>
...
<template>
  <AdminCrudView
    type="events"
    title="活动管理"
    description="创建活动并维护海报、时间、场地、容量和状态，支持查看报名概览与活动状态流转。"
    singular="活动"
    :columns="columns"
    :fields="fields"
    :stats="stats"
