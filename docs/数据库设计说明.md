...
后台报名名单可按状态筛选、更新状态并导出 CSV，相关操作写入 `audit_logs`。
...
### user_penalties / user_risk_logs
社区处罚与用户风控日志表。`user_penalties` 记录禁言、限制发帖、永久限制和恢复操作；`user_risk_logs` 记录风控流水。社区发帖和评论会读取用户限制状态并在限制期内拒绝提交。
...
服务端会按白名单时间段、维护座位、用户预约限制和同座位时间冲突进行二次校验。
...
- 上传服务会拒绝危险扩展名、SVG、危险双扩展名和文件魔数不匹配内容。
...
- `payments.expires_at`
- `user_notifications`
- `event_registrations.status/registered_at/cancelled_at/attended_at/absent_at`
- `user_penalties`
- `user_risk_logs`
- `users.post_limit_until`
- `users.booking_limit_until`
