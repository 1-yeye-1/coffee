...
const progress = computed(() => Math.min(100, Math.round(membershipStore.account.growth / membershipStore.account.nextLevelGrowth * 100)))
const benefits = [
...
<template>
  <div class="member-page">
    ...
    <section class="member-panel"><div class="member-panel__header"><div><BaseBadge variant="premium">{{ membershipStore.account.level }}</BaseBadge><h3>成长值 {{ membershipStore.account.growth }}</h3></div><strong>{{ progress }}%</strong></div><div class="progress-track"><span :style="{ width: `${progress}%` }" /></div><p class="text-muted">距离下一等级还需 {{ membershipStore.account.nextLevelGrowth - membershipStore.account.growth }} 成长值。</p></section>
