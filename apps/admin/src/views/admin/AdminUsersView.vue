...
function openRisk(item, type) {
  ...
  const titleMap = {
    ...
    booking: isBookingRestricted ? '解除预约限制' : '限制预约',
    post: isPostRestricted ? '解除发帖限制' : '限制发帖',
    points: '积分调整',
  }
  Object.assign(riskForm, { type, title: titleMap[type], reason: '', pointsDelta: 0, endAt: '', target: item })
  riskOpen.value = true
}

function riskLabel(riskType) {
  const map = {
    disable: '禁用账号', enable: '解禁账号',
    booking_limit: '限制预约', booking_unlimit: '解除预约限制',
    post_limit: '限制发帖', post_unlimit: '解除发帖限制',
    community_penalty: '社区处罚',
    mute_1d: '禁言 1 天', mute_7d: '禁言 7 天',
    post_ban: '限制发帖', permanent_post_ban: '永久限制发帖',
    restore: '恢复权限',
  }
  return map[riskType] || riskType || '风控操作'
}

function openRiskFromLog(log) {
  const user = userDetail.value
  if (!user) return
  const riskType = String(log.riskType || '')
  if (riskType.includes('booking')) {
    openRisk(user, 'booking')
  } else if (riskType.includes('post') || riskType.includes('mute') || riskType.includes('penalty') || riskType.includes('ban')) {
    openRisk(user, 'post')
  } else {
    openRisk(user, 'disable')
  }
  ...
  <div class="admin-page">
    ...
    <BaseDrawer v-model="detailOpen" :title="userDetail?.nickname || userDetail?.username || '用户详情'" @close="userDetail = null">
      ...
      <div v-else-if="userDetail" class="user-detail">
        ...
        <section>
          <h3>风控记录</h3>
          <article v-for="log in userDetail.riskLogs || []" :key="log.id" class="user-log">
            <div class="user-log__head">
              <strong>{{ riskLabel(log.riskType) }}</strong>
              <small>{{ formatDate(log.createdAt) }}</small>
            </div>
            <span>原因：{{ log.reason || '-' }}</span>
            <span v-if="log.operatorId">操作人 ID：{{ log.operatorId }}</span>
