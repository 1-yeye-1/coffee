...
<<<<<<< HEAD
  ...
  router.patch('/api/admin/books/:bookId/reservations/:reservationId/status', requireAdmin, async (req, res) => {
    if (!['pending', 'confirmed', 'completed', 'cancelled'].includes(req.body.status)) return failure(res, 400, '图书预约状态无效')
    const reservation = await updateAdminBookReservationStatus(req.params.bookId, req.params.reservationId, req.body, req.user.id)
    if (!reservation) return failure(res, 404, '图书不存在', 404)
